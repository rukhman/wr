...
class AuthController {
  async signup(req, res) {
    try {
      ...
      if (!user.verified) {
        // при разработке на локальной машине email отправляться не смогут
  ...
  async addToFavorite(req, res) {
    try {
      const product = await fn.addOneTo(
        req.body.product,
        req.user.id,
        "in_favorite"
      );
  ...
  async deleteFromFavorite(req, res) {
    try {
      const product = await fn.deleteOneFrom(
        req.query.product,
        req.user.id,
        "in_favorite"
      );
  ...
  async editProduct(req, res) {
    try {
      const images = await fn.editProductImages(
        req.body.imageData,
        req.body.id
      );
