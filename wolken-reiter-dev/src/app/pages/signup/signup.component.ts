  ...
  template: `
    <form [formGroup]="checkoutForm" [ngStyle]="{width: hostWidth}" class="form" (ngSubmit)="sendForm()">
      ...
      <div class="form__buttons">
        <a class="form__login" mat-stroked-button routerLink="/login">{{ "FORM.LOGIN" | translate }}</a>
        <button color="warn" class="form__signup" type='button' mat-stroked-button>{{ "FORM.SIGNUP" | translate }}</button>
        <button color="primary" class="form__submit" mat-raised-button>{{ "FORM.SUBMIT" | translate }}</button>
      </div>
    </form>
  `,
