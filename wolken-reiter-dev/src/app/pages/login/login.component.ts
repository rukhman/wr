  ...
  template: `
    <form [formGroup]="checkoutForm" [ngStyle]="{width: hostWidth}" class="form" (ngSubmit)="sendForm()">
      ...
      <div class="form__buttons">
        <button type="button" color="warn" class="form__login" mat-stroked-button>{{ "FORM.LOGIN" | translate }}</button>
        <a class="form__signup" mat-stroked-button routerLink="/signup">{{ "FORM.SIGNUP" | translate }}</a>
        <button mat-raised-button color="primary" class="form__submit">{{ "FORM.SUBMIT" | translate }}</button>
      </div>
    </form>
  `,
