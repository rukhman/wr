...
export class AuthService {
  ...
  sendSignupForm(form: SignupData): any {
    return this.createUser(form).pipe(
      mergeMap((resp: UserCreatedResponse | undefined) => {
        if (resp?.userCreated) {
          ...
          return this.sendLoginForm(data);
        }
        return new Observable();
      }),
      catchError((err) => {
  ...
  checkEmail = (
    email: string
  ): Observable<EmailExistsResponse | EmailNotExistsResponse> => {
    const url = this.baseUrl + '/check';
    const data: Email = { email };
    return this.http.post<EmailExistsResponse | EmailNotExistsResponse>(
      url,
      data
    );
  };
